...
ENV USER=root
RUN apt-get update && apt-get install -y gnupg apt-transport-https wget software-properties-common ratpoison novnc websockify libxv1 libglu1-mesa xauth x11-utils xorg tightvncserver
...
EXPOSE 80
CMD x11vnc -display :0 -nopw -listen localhost -forever & websockify -D --web=/usr/share/novnc/ --cert=~/.novnc/novnc.pem 80 localhost:5900 && openbox & tail -f /dev/null
